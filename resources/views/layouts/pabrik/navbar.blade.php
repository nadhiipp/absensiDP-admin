<header class="fixed top-0 left-60 right-0 h-20 bg-white border-b border-slate-200 flex items-center justify-end px-8 z-40">
    <!-- Right Section -->
    <div class="flex items-center gap-5">
        <!-- Notification -->
        <button class="relative w-10 h-10 flex items-center justify-center rounded-lg text-slate-500 hover:bg-slate-100 transition-all">
            <i class="bi bi-bell text-xl"></i>
            <span class="absolute top-2 right-2 w-2 h-2 bg-red-500 rounded-full"></span>
        </button>

        <!-- User -->
        <div class="flex items-center gap-3 cursor-pointer">
            <div class="text-right">
                <div class="text-sm font-semibold text-slate-800">Admin Pabrik</div>
                <div class="text-xs text-slate-400">Administrator</div>
            </div>
            <div class="w-10 h-10 bg-emerald-500 rounded-full flex items-center justify-center text-white text-sm font-semibold">
                AP
            </div>
        </div>
    </div>
</header>