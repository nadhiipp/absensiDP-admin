<aside class="fixed top-0 left-0 w-60 h-screen bg-white border-r border-slate-200 z-50 flex flex-col">
    ...
    <div class="flex items-center gap-3 px-5 py-5 border-b border-slate-200">
        ...
        <span class="font-semibold text-slate-800">Absensi kantor</span>
    </div>
    ...
    <nav class="flex-1 p-3 overflow-y-auto">
        ...
        <a href="{{ route('admin-kantor.data-pegawai') }}" 
           class="flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium mb-1 transition-all duration-200 no-underline
                  {{ request()->routeIs('admin-kantor.data-pegawai') 
                     ?  'bg-blue-50 text-blue-600' 
                     : 'text-slate-500 hover:bg-slate-50 hover:text-slate-700' }}">
        ...
        <a href="{{ route('admin-kantor.absensi') }}" 
           class="flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium mb-1 transition-all duration-200 no-underline
                  {{ request()->routeIs('admin-kantor.absensi') 
                     ? 'bg-blue-50 text-blue-600' 
                     : 'text-slate-500 hover:bg-slate-50 hover: text-slate-700' }}">
        ...
        <a href="{{ route('admin-kantor.rekap-absensi') }}" 
           class="flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium mb-1 transition-all duration-200 no-underline
                  {{ request()->routeIs('admin-kantor.rekap-absensi') 
                     ? 'bg-blue-50 text-blue-600' 
                     : 'text-slate-500 hover:bg-slate-50 hover:text-slate-700' }}">
