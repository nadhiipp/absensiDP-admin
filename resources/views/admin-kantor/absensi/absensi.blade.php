...
<div class="bg-white border border-slate-200 rounded-xl overflow-hidden">
    <div class="px-5 py-4 border-b border-slate-200 flex flex-wrap justify-between items-center gap-3">
        ...
        <div class="flex flex-wrap gap-3">
            <div class="relative">
                ...
                <input type="text" 
                       ...
                       class="w-64 pl-11 pr-4 py-2 border border-slate-200 rounded-lg text-sm bg-white focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100 transition-all">
            </div>
            ...
            <button onclick="openModal()" class="px-4 py-2 bg-blue-500 hover:bg-blue-600 text-white rounded-lg text-sm font-medium transition-all inline-flex items-center gap-2">
        ...
        <table class="w-full">
            <thead class="bg-slate-50 border-b border-slate-200">
                <tr>
                    ...
                    <th class="text-center px-5 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wide">Aksi</th>
                </tr>
            </thead>
            <tbody class="divide-y divide-slate-100">
                ...
                <tr class="hover: bg-slate-50 transition-all">
                    ...
                    <td class="px-5 py-4 text-sm font-medium text-blue-600">{{ $a['nip'] }}</td>
                    ...
                    <td class="px-5 py-4">
                        <div class="flex items-center justify-center gap-1">
                            <!-- Edit Button - PERHATIKAN PARAMETER NYA -->
                            <button onclick="openModalEdit('{{ $a['no'] }}', '{{ $a['nip'] }}', '{{ $a['nama'] }}', '{{ $a['dept'] }}', '{{ $a['shift'] }}', '{{ $a['masuk'] }}', '{{ $a['keluar'] }}', '{{ $a['status'] }}', '{{ $a['ket'] }}')" 
                                    class="w-8 h-8 inline-flex items-center justify-center rounded-lg text-slate-400 hover:bg-blue-50 hover: text-blue-600 transition-all cursor-pointer"
                                    title="Edit">
                                <i class="bi bi-pencil-square"></i>
                            </button>
                            <!-- Delete Button - PERHATIKAN PARAMETER NYA -->
                            <button onclick="openModalDelete('{{ $a['no'] }}', '{{ $a['nip'] }}', '{{ $a['nama'] }}', '{{ $a['shift'] }}', '{{ $a['masuk'] }}', '{{ $a['status'] }}')" 
                                    class="w-8 h-8 inline-flex items-center justify-center rounded-lg text-slate-400 hover:bg-red-50 hover: text-red-500 transition-all cursor-pointer"
                                    title="Hapus">
    ...
    <div class="px-5 py-4 border-t border-slate-200 flex items-center justify-between">
        <p class="text-sm text-slate-500">Menampilkan 1-10 dari 324 data</p>
        <div class="flex items-center gap-2">
            <button class="px-3 py-1.5 border border-slate-200 rounded-lg text-sm text-slate-500 hover:bg-slate-50 transition-all">
                <i class="bi bi-chevron-left"></i>
            </button>
            <button class="px-3 py-1.5 bg-blue-500 text-white rounded-lg text-sm font-medium">1</button>
            <button class="px-3 py-1.5 border border-slate-200 rounded-lg text-sm text-slate-600 hover:bg-slate-50 transition-all">2</button>
            <button class="px-3 py-1.5 border border-slate-200 rounded-lg text-sm text-slate-600 hover: bg-slate-50 transition-all">3</button>
            <span class="text-slate-400">... </span>
            <button class="px-3 py-1.5 border border-slate-200 rounded-lg text-sm text-slate-600 hover:bg-slate-50 transition-all">33</button>
            <button class="px-3 py-1.5 border border-slate-200 rounded-lg text-sm text-slate-500 hover:bg-slate-50 transition-all">
...
<!-- Include Modals -->
@include('admin-kantor.absensi.create')
@include('admin-kantor.absensi.edit')
@include('admin-kantor.absensi.delete')
@endsection